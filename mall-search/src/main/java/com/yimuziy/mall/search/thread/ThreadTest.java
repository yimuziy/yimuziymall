package com.yimuziy.mall.search.thread;
...
//            System.out.println("任务3开始。。。之前的结果"+t);
//        },executor);
...
//        new Thread(() -> System.out.println("hello")).start();
        ...
        /**
         * 七大参数：
         *  corePoolSize：【5】核心线程数[一直存在除非（allowCoreThreadTimeOut）]；线程池，创建好以后就准备就绪的线程数量，就等待来接收异步任务去执行。
         ...
         *          3、max都执行好了。Max-core数量空闲的线程会在keppAliveTime指定的时间后自动销毁。最终保持到core大小
         ...
         *
         *
         *  面试：一个线程池 core7： max 20， queue： 50 ，100并发进来怎么去分配的
         *  7个会立即执行，50个会进入队列，再开13个进行执行。剩下的30个就是用拒绝策略。
         *  如果不想抛弃还要执行 java.util.concurrent.ThreadPoolExecutor.CallerRunsPolicy
         */
